...
{
    internal class ObjectFileParser: IObjectParser<FileObjectData, FileObjectDataDto>
    {
        private readonly AssemblyHelper _assemblyHelper;
        private readonly IObjectContainer<FileObjectData> _objectContainer;

        public ObjectFileParser(IObjectContainer<FileObjectData> objectContainer, AssemblyHelper assemblyHelper)
        {
        ...
        public FileObjectData? CreateObjectData(FileObjectDataDto dto)
        {
            ...
            var concurrentBag = new ConcurrentBag<FilePropertyData>();
        ...
        public FileObjectDataDto GetObjectDataDto(FileObjectData objectData)
        {
            var dto = new FileObjectDataDto()
            {
            ...
            var concurrentBag = new ConcurrentBag<FilePropertyDataDto>();

            var tasks = objectData.Properties.Select(x =>
            {
                return Task.Run(() => concurrentBag.Add(new FilePropertyDataDto
                {
                    Name = x.Name,
        ...
        private FilePropertyData? CreatePropertyData(FilePropertyDataDto dto)
        {
            if (dto is null)
            {
                return null;
