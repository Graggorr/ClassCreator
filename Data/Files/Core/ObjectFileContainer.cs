...
{
    internal class ObjectFileContainer : IObjectContainer<FileObjectData>
    {
        ...
        public bool SaveOrUpdate(FileObjectData objectData)
        {
        ...
        public FileObjectData? Get(string typeName) => GetObjectDataFromFile(GetFullPath(typeName));

        public bool Contains(FileObjectData objectData) => Get(objectData.Name) is not null;

        public IEnumerable<FileObjectData> GetAll()
        {
            var concurrentBag = new ConcurrentBag<FileObjectData>();
        ...
        public bool Remove(FileObjectData ObjectData, CancellationToken cancellationToken = default)
        {
        ...
        private static FileObjectData? GetObjectDataFromFile(string path)
        {
            if (!Path.Exists(path))
            {
                return null;
            ...
            return JsonConvert.DeserializeObject<FileObjectData>(data);
        }
